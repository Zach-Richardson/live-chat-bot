  ...
  static async onboard(onboardClient) {
    ...
    if (this.onboardingCreatedUser) {
      try {
        botUser = await onboardClient.fetch("/v1/user/", {
          method: "POST",
          json: Object.assign({}, this.onboardingCreatedUser, {
            phone: botUser.phone,
            email: botUser.email,
            user_type: "BOT"
          })
    ...
    try {
      ...
      const result = await Promise.race([
        Promise.all(["done", something.done]),
        Promise.all(["timeout", relay.util.sleep(15)])
      ]);
      if (result[0] === "timeout") throw "registerDevice timed out";
      console.log("registerDevice success");
    } catch (e) {
      console.log(
        "registerDevice didn't work out, trying registerAccount instead"
      );
