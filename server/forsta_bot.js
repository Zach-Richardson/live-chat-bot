...
class ForstaBot {
  async start() {
    ...
    //bot
    this.getQuestions = cache.ttl(120, () =>
      relay.storage.get("live-chat-bot", "questions")
    );
    this.getGroups = cache.ttl(120, () =>
      relay.storage.get("live-chat-bot", "groups")
    );
    this.threadStatus = {};
    this.sockets = {};
  }
  ...
  async configureSocket(io) {
    let addSocket = function(socket, userId) {
      console.log("configuring socket for id:" + userId);
    ...
    let connectOperator = async function(operatorId, threadId, timeConnected) {
      try {
        ...
        let ephemeralUser = (await this.getUsers([
          this.threadStatus[threadId].user.id
        ]))[0];
        ...
        this.sendMessage(
          this.threadStatus[threadId].dist,
          threadId,
  ...
  async onMessage(event) {
    ...
    if (
      !msg ||
      (msg.messageType == "control" && msg.data.control == "readMark")
    ) {
  ...
  async initializeNewThread(msg) {
    ...
    const ephUser = (await this.getUsers(dist.userids)).filter(
      user => user.id != this.ourId
    )[0];
    const botUser = (await this.getUsers(dist.userids)).filter(
      user => user.id == this.ourId
    )[0];
  ...
  async stepThreadState(threadStatus, msg) {
    ...
    let { action, actionOption } =
      threadStatus.currentQuestion.type == "Multiple Choice"
    ...
    if (action == "Forward to Question") {
      ...
      const operatorConnectMessage =
        "A live chat operator will be with you shortly";
      let outgoingMessage = await this.sendMessage(
        threadStatus.dist,
        threadStatus.threadId,
        operatorConnectMessage
      );
      let messageBody = JSON.parse(outgoingMessage.message.dataMessage.body)[0];
      await this.saveToThreadMessageHistory(messageBody);
      let group = (await this.getGroups()).find(
        group => group.name == actionOption
      );
  ...
  async sendQuestion(dist, threadId, question) {
    ...
    if (question.type == "Multiple Choice") {
      const actions = question.responses.map((response, index) => {
        return { title: response.text, color: response.color, action: index };
      });
      outgoingMessage = await this.sendActionMessage(
        dist,
        threadId,
        question.prompt,
        actions
  ...
  async saveToThreadMessageHistory(message) {
    ...
    let text = message.data.body
      ...
      : threadStatus.currentQuestion.responses[parseInt(message.data.action)]
          .text; //if its an incoming message save the response text based on the selected action
  ...
  forgetStaleNotificationThreads() {
    ...
    console.log(
      "stale notification threads removed. currently tracking:",
      Object.assign({}, this.notificationThread)
    );
  }

  async incrementAuthFailCount() {
    let fails = await relay.storage.get("authentication", "fails", {
      count: 0,
      since: new Date()
    });
    fails.count++;
    ...
    if (fails.count >= AUTH_FAIL_THRESHOLD) {
      await this.broadcastNotice({
        note: `SECURITY ALERT!\n\n${
          fails.count
        } failed login attempts (last successful login was ${moment(
          fails.since
        ).fromNow()})`
      });
    }
  ...
  async resetAuthFailCount() {
    await relay.storage.set("authentication", "fails", {
      count: 0,
      since: new Date()
    });
  }
  ...
  async sendAuthCode(tag) {
    ...
    if (resolved.userids.length === 1 && resolved.warnings.length === 0) {
      ...
      throw {
        statusCode: 400,
        info: { tag: ["not a recognized tag, please try again"] }
      };
    }
  }
  ...
  async validateAuthCode(userId, code) {
    ...
    if (!auth) {
      throw {
        statusCode: 403,
        info: { code: ["no authentication pending, please start over"] }
      };
    }
    if (auth.code != code) {
      this.incrementAuthFailCount();
      await relay.util.sleep(0.5); // throttle guessers
      throw {
        statusCode: 403,
        info: { code: ["incorrect codewords, please try again"] }
      };
    }
  ...
  async broadcastNotice({ note, actorUserId, listAll = true }) {
    ...
    const adminList = adminUsers
      .filter(u => !(added && u.id === actorUserId))
      .map(u => this.fqLabel(u))
      .join("\n");
  ...
  async addAdministrator({ addTag, actorUserId }) {
    ...
    if (resolved.userids.length === 1 && resolved.warnings.length === 0) {
      ...
      await this.broadcastNotice({
        note: `ADDED <<${uid}>> to authorized users`,
        actorUserId
      });
  ...
  async removeAdministrator({ removeId, actorUserId }) {
    ...
    await this.broadcastNotice({
      note: `REMOVING <<${removeId}>> from authorized users`,
      actorUserId
    });
    await relay.storage.set("authentication", "adminIds", adminIds);
