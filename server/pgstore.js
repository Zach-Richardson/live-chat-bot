...
class PGStore {
  ...
  async beginProtectedTransaction() {
    ...
    if (result.length !== 2 || result[1].rowCount !== 1)
      throw new Error("Failure in postgres protected-transaction lock");
  }
  ...
  async getMessages({
    ...
    chainCorruption,
    mainCorruption,
    attachmentsCorruption,
    previousCorruption
  }) {
    const _selectfrom = `SELECT *, count(*) OVER() AS full_count FROM ${
      this.prefix
    }_message`;
    ...
    let predicates = [];
    if (until)
      predicates.push(`received <= '${until}'::timestamp with time zone`);
    if (since)
      predicates.push(`received >= '${since}'::timestamp with time zone`);
    ...
    if (attachments === "yes")
      predicates.push("array_length(attachment_ids, 1) > 0");
    ...
    const _orderby = orderby
      ? `ORDER BY ${orderby} ${ascending === "yes" ? "ASC" : "DESC"}`
      : "";
